...
function disarmHTML(htmlString) {
    ...
    function getUnclobberedGetter(obj, prop) {
        let current = obj;
        while (current) {
    ...
    const getTagName = getUnclobberedGetter(safeEl, 'tagName');
    const getAttributes = getUnclobberedGetter(safeEl, 'attributes');
    ...
    function processRoot(root) {
        ...
        while (el) {
            ...
            if (el.nodeType === 1) {
                const tagName = getTagName.call(el);

                if (activeTags.has(tagName.toLowerCase())) {
                    ...
                    if (tagName.toLowerCase() === 'template' && el.content) {
