...
// ⚡ Bolt Optimization: Use a precompiled global regex for significantly faster URL extraction compared to manual string scanning.
...
function extractUrls(text) {
    ...
    while ((match = URL_EXTRACT_REGEX.exec(text)) !== null) {
        ...
        // Ensure the stripped URL is still valid and not just 'http://'
        if (url.length > 7) {
            urlSet.add(url);
        }
    }
