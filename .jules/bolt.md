...
## 2024-11-06 - Replacing regex string replace with string iteration in hot loop
**Learning:** Using regex `.replace(/[.,;:!)\]]+$/, '')` to strip trailing characters allocates matched arrays and incurs regex execution overhead. A native `while` loop starting from the string's end and checking `indexOf` in a defined punctuation string (`.,;:!)]`) is significantly faster. Combining this with standard Array `push()` and `.indexOf()` duplicate-checks (instead of Set allocations) creates a much faster hot-path parsing function.
**Action:** Replace simple string termination replacements and small unique collections with native string and array operations instead of Regular Expressions and `new Set`s in functions executed frequently, like URL extraction.
