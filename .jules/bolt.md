...
## 2024-09-04 - Regex vs Manual String Scanning
**Learning:** Manual string scanning loops (with `indexOf` and `charCodeAt`) can be much slower than well-crafted global regular expressions in V8, since the regex engine is highly optimized C++ code that easily handles character boundaries.
**Action:** Prefer well-crafted regular expressions over complex, manual scanning loops when extracting patterns from large strings on hot paths.
