...
## 2024-06-21 - VM Context Array.from Prototype Mismatch\n**Learning:** When testing code evaluated via Node's `vm` context, arrays or objects returned by the VM (such as those returned by `Array.from()` inside the evaluated code) do not share the same prototype as arrays created in the test suite context. This causes `assert.deepStrictEqual` to fail with reference-equality errors even when the structures are identical.\n**Action:** When testing VM-evaluated arrays, weaken assertions from `assert.deepStrictEqual` to `assert.deepEqual` to properly validate structural equivalence without prototype mismatches.\n
