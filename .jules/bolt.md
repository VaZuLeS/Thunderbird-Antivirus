...
**Learning:** Attempting to optimize `new URL(url).hostname` by manually slicing strings (e.g. using `indexOf('/')`, `indexOf('?')`) in a hot loop is highly error-prone and can introduce severe vulnerabilities or break functionality by mismatching hostnames and query params.
**Action:** When extracting hostnames for performance in a hot loop, do not use error-prone manual `indexOf` loops. Instead, prefer a robust precompiled regex (like `/^(?:https?:\/\/)([^\/\?#\\]+)/i`) that correctly handles basic syntax and escapes. Check the resulting match for edge cases (like `@` for auth, `:` for ports/IPv6) and fallback to the robust native `new URL()` parser only when necessary.
