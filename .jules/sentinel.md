...
## 2026-05-25 - Javascript URI evasion via Space and Unicode Replacement
**Vulnerability:** The HTML sanitizer (`disarmHTML`) was attempting to strip javascript URI evasions by removing control characters (e.g. `\x00-\x1F`), but missed standard spaces (`\x20`) and the Unicode Replacement Character (`\uFFFD`). This allowed payloads like `java script:alert(1)` or `javascript:alert(1)` to bypass the `startsWith` check, leading to Mutation XSS.
**Learning:** Browsers are highly permissive when parsing URIs and will ignore many characters, including spaces and certain Unicode artifacts, inside the scheme (e.g., before the colon). A naive `trim()` or removing only control characters is insufficient to neutralize all evasions.
**Prevention:** When sanitizing potentially dangerous attributes against known bad schemes, always aggressively strip spaces and unicode replacement characters alongside control characters *before* performing substring matching (`startsWith`), or rely on a robust URL parsing library rather than string manipulation.
