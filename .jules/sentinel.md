...
## 2026-07-06 - Insecure URL Parsing via Regex

**Vulnerability:** The \`getHostnameOptimized\` function in \`background.js\` used an unsafe Regular Expression (\`/^(?:https?:\\/\\/)([^\\/\\?#\\\\]+)/i\`) to manually extract hostnames from URL strings. This manual extraction violates standard URL specifications (RFC 3986) and can be easily bypassed by attackers using complex payloads (such as unexpected delimiters, backslashes, or encoded sequences). This spoofing vulnerability could allow attackers to trick the application into trusting malicious domains or incorrectly handling security evaluations.
**Learning:** Manual string operations (like Regex, \`indexOf\`, or \`substring\`) should never be used to parse URLs or extract components like hostnames or protocol schemas. Browser-native URL parsers are highly complex and fault-tolerant in ways that simple regexes cannot safely replicate.
**Prevention:** Always strictly use the built-in \`new URL(url)\` object for URL parsing. Wrap the instantiation in a \`try/catch\` block to safely handle and reject invalid or malformed URL strings, and ensure all calling code locations are updated to perform proper null/undefined checks on the returned values to prevent \`TypeError\` exceptions.
