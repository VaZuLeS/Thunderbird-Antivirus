...
## 2026-09-13 - DOM Clobbering in HTML Sanitizer
**Vulnerability:** The HTML sanitizer (`disarmHTML` in `background.js`) relied on dot-notation DOM attribute accessors like `el.tagName`, `el.attributes`, and `el.hasAttributes()`. Attackers could inject elements like `<form><input name="hasAttributes"></form>` to "clobber" the DOM API, potentially bypassing XSS filters and causing JavaScript errors or security failures.
**Learning:** Browser native DOM element properties and methods can be trivially overridden by child elements with specific `name` or `id` attributes in certain contexts (like forms).
**Prevention:** When traversing untrusted HTML documents, always extract properties using descriptors from the unclobbered prototype chain (`Object.getOwnPropertyDescriptor(Object.getPrototypeOf(safeEl), ...).get.call(el)`) and call prototype methods securely (`safeEl.removeAttribute.call(el, ...)`).
