...
## 2026-07-15 - Unsafe URL Hostname Extraction
**Vulnerability:** URL hostname parsing relied on manual regex extraction (`HTTP_HOSTNAME_EXTRACTOR`) instead of the standardized `new URL()`. This introduces critical hostname spoofing vulnerabilities where malicious payloads (e.g. involving backslashes like `https://google.com\@evil.com`) are incorrectly evaluated as the trusted domain instead of the true target domain.
**Learning:** URL specifications are complex. Do not use manual string operations or regex to parse URLs or extract hostnames, as this explicitly breaks specifications and creates severe security vulnerabilities.
**Prevention:** Always strictly use the built-in `new URL(url).hostname` wrapped in a `try/catch` block for safe and compliant URL parsing.
