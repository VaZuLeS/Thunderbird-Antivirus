...
## 2026-07-01 - Fix Unhandled ReferenceError in UI rendering (setElementText)
...
## 2026-08-16 - DOM Clobbering Vulnerability in disarmHTML
**Vulnerability:** The `disarmHTML` function in `background.js` was susceptible to DOM Clobbering. By injecting elements with attributes like `name="tagName"`, `name="attributes"`, or `name="hasAttributes"` (e.g. `<form><img name="tagName"><input name="hasAttributes"></form>`), an attacker could overwrite native properties of the `el` object. This could bypass security checks (e.g., throwing a `TypeError: el.hasAttributes is not a function`), which allowed malicious payloads to bypass sanitization altogether.
**Learning:** Native DOM element properties and methods like `tagName`, `hasAttributes`, and `attributes` can be easily overwritten (clobbered) by properties derived from named child elements in older DOM elements like `<form>`. Using dot-notation directly on untrusted DOM elements (e.g., `el.hasAttributes()`) is extremely dangerous when parsing untrusted HTML.
**Prevention:** Always extract and apply safe methods from a clean, untainted element (e.g., `const safeEl = document.createElement('div')` and `safeEl.hasAttributes.call(el)`). For getter properties like `tagName`, safely resolve the getter from the un-clobbered prototype chain using `Object.getPrototypeOf()` and `Object.getOwnPropertyDescriptor()` rather than relying on direct property access.
