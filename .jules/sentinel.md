...
## 2024-08-18 - Email Spoofing via Duplicate Extraction Logic
**Vulnerability:** The `tab_mail_open_display` function in `background.js` used a vulnerable `indexOf('<')` implementation to extract the sender's email address for opt-in verification. This logic was susceptible to email spoofing via decoy display names (e.g., `"Safe Sender <decoy@safe.com>" <hacker@evil.com>`), potentially allowing auto-upload bypass.
**Learning:** Security vulnerabilities often reappear when secure utility functions (like `extractEmailAddress` which correctly uses `lastIndexOf`) are not consistently reused across the codebase, and developers rewrite ad-hoc string parsing logic.
**Prevention:** Consistently use centralized, well-tested security utility functions for data parsing and validation instead of duplicating logic. Enforce this via code reviews or static analysis rules.
