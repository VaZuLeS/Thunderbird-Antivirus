...
## 2024-11-04 - Focus Trap in Dialog Modals
**Learning:** Native or custom dialogs and modals (like the warning and loading overlays injected via content scripts) do not automatically trap keyboard focus. If a user presses `Tab`, the focus can escape the modal and navigate to links and buttons in the obscured background page, violating WCAG 2.4.3 (Focus Order) and causing significant confusion for keyboard and screen reader users.
**Action:** To comply with WCAG 2.4.3 and improve accessibility, always implement a keyboard focus trap inside custom vanilla JS modals or dialogs to prevent tabbing into the inert background. For modals with interactive elements, handle `Tab` and `Shift+Tab` to cycle focus. For modals without interactive elements, simply prevent the `Tab` event's default action.
