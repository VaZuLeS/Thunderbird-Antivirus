...
## 2024-09-12 - ARIA Describedby for Async Button Feedback
**Learning:** Action buttons that trigger asynchronous operations (like saving settings or uploading files) and output their results to a nearby status element can leave screen reader users disconnected from the result container. While `aria-live` announces changes, explicitly linking the button to the status container using `aria-describedby` provides immediate context upon focusing the button that feedback will appear there.
**Action:** Always link action buttons to their corresponding dynamic status or feedback containers (e.g., `aria-describedby="status-id"`) to provide comprehensive context for assistive technologies.
