...
## 2026-07-04 - Progress Cursors for Async Buttons
**Learning:** In standard setups, disabling a button sets the cursor to `not-allowed`. However, for buttons disabled *because* an asynchronous task is loading (indicated by `aria-busy="true"`), a `not-allowed` cursor communicates to the user that they did something wrong or an action is forbidden, rather than communicating the system is working.
**Action:** When styling interactive elements in `theme.css`, add a specific rule `button[aria-busy="true"]:disabled { cursor: progress; }` to override the default disabled cursor and provide proper mental model feedback during async operations.
