...
## 2026-06-15 - Explicit Disabled States for Input Elements\n**Learning:** In options pages where fields like `select` or `checkbox` are dynamically disabled via JavaScript based on other selections, relying solely on the native `disabled` state without visual indicators leaves sighted users confused. Explicitly styling `:disabled` pseudo-classes for inputs creates a cohesive UI.\n**Action:** When implementing custom themes (e.g., `theme.css`), always ensure `input:disabled`, `select:disabled`, and `textarea:disabled` are styled alongside `button:disabled` (e.g., with reduced opacity and a not-allowed cursor) for consistent and clear visual feedback.
